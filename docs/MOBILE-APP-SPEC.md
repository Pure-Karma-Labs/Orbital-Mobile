...
Signal does not publish libsignal-ffi as a versioned, stable API. Breaking changes can occur between releases. **Mitigation:** Pin to a specific libsignal version (v0.83.0 at time of writing; current pin: v0.102.3) and only upgrade deliberately.
...
- [ ] Create thin Rust crate wrapping libsignal v0.102.3 (pin version)
...
| libsignal has no stable public FFI API | High | Pin to v0.102.3, wrap minimal surface, upgrade deliberately |
