...
[ "${#FILES[@]}" -gt 0 ] || fail "no podspec JSON files under ${PODS_DIR}/Local Podspecs" "This assertion must not pass vacuously."
...
if [ "$OUTCOME" = "success" ]; then
  ...
  [ "${#FILES[@]}" -gt "$N_BEFORE" ] || fail "no xcconfig files under ${PODS_DIR}/Target Support Files" "This assertion must not pass vacuously."
else
